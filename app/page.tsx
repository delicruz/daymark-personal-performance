...
function AiDailyCoach({ data, authenticated, ready, todayKey }: { data: DaymarkData; authenticated: boolean; ready: boolean; todayKey: string }) {
  ...
  const cacheKey = `daymark-ai-daily-plan:v5:${data.user.id}:${todayKey}`;
...
function Insights({ setModal, data }: { setModal: (modal: Modal) => void; data: DaymarkData }) {
  ...
  const tickDates = range === 7
    ? snapshot.dates
    : [snapshot.dates[0], snapshot.dates[Math.floor((snapshot.dates.length - 1) / 2)], snapshot.dates.at(-1)!];
