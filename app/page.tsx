...
type CalendarSummary = { summaryDate: string; meetingCount: number; meetingMinutes: number; focusMinutes: number; syncedAt: string };
...
type DaymarkData = { user: DaymarkUser; profile: UserProfile | null; checkins: CheckinRecord[]; latestMorning: CheckinRecord | null; priorities: PriorityRecord[]; calendarSummaries: CalendarSummary[]; forecast: number; forecastModel: ForecastModel; baselineDays: number };
...
const faqs = [
  ...
  { category: "Calendar", question: "Is calendar data used in the model?", answer: "Calendar connection now creates descriptive insights from daily meeting count, scheduled minutes and open work time. It does not affect the tested prediction model yet; calendar variables need separate forward-only validation before they can influence forecasts." },
...
function Today({ setModal, data, onAddPriority, onTogglePriority, onCalendarConnect, onOpenInsights }: { setModal: (modal: Modal) => void; data: DaymarkData; onAddPriority: (title: string) => Promise<boolean>; onTogglePriority: (id: number, completed: boolean) => Promise<void>; onCalendarConnect: () => Promise<void>; onOpenInsights: () => void }) {
  ...
  const tomorrowCalendar = data.calendarSummaries.find((summary) => summary.summaryDate === tomorrowKey);
    ...
    <>
      ...
      <div className="dashboard-grid">
        ...
        <section className="calendar-card">
          <div className="card-label">TOMORROW’S CALENDAR <button onClick={() => void onCalendarConnect()}>{connected ? "Sync again ↗" : "Connect Google ↗"}</button></div>
          {connected ? <><div className="calendar-stats"><div><strong>{tomorrowCalendar?.meetingCount ?? 0}</strong><span>Meetings</span></div><div><strong>{formatMinutes(tomorrowCalendar?.meetingMinutes ?? 0)}</strong><span>Scheduled time</span></div><div><strong>{formatMinutes(tomorrowCalendar?.focusMinutes ?? 480)}</strong><span>Open work time</span></div></div><div className="calendar-insight"><span>✦</span><p><small>CALENDAR INSIGHT</small><strong>{calendarInsight.text}</strong></p></div></> : <div className="calendar-connect"><span className="calendar-logo">31</span><div><strong>Connect Google Calendar</strong><p>Turn meeting load into private daily summaries. Event names and people are never stored.</p></div><button className="dark-button" onClick={() => void onCalendarConnect()}>Connect</button></div>}
        </section>
...
function Settings({ setModal, data, onCalendarConnect, onCalendarDisconnect, onProfileUpdate, onDelete }: { setModal: (modal: Modal) => void; data: DaymarkData; onCalendarConnect: () => Promise<void>; onCalendarDisconnect: () => Promise<void>; onProfileUpdate: (displayName: string, goal: string) => Promise<void>; onDelete: () => Promise<void> }) {
  ...
  return <><AppHeader title="Your data, your choices." subtitle="DATA & SETTINGS" setModal={setModal} /><div className="settings-layout"><div className="settings-tabs" aria-label="Settings sections" role="tablist" aria-orientation="vertical">{sections.map((section) => <button key={section.id} id={`settings-tab-${section.id}`} role="tab" aria-selected={activeSection === section.id} aria-controls={`settings-panel-${section.id}`} className={activeSection === section.id ? "active" : ""} onClick={() => setActiveSection(section.id)}>{section.label}</button>)}</div><div className="settings-content" id={panelId} role="tabpanel" aria-labelledby={`settings-tab-${activeSection}`}>
    ...
    {activeSection === "integrations" && <section className="settings-section"><div><h2>Calendar connection</h2><p>Daymark only stores daily meeting counts, scheduled minutes and open work time. Titles, descriptions, attendees, locations and Google access tokens are never stored in Daymark’s database.</p></div><div className="integration-row"><span className="calendar-logo">31</span><div><strong>Google Calendar</strong><small>{connected ? `${data.calendarSummaries.length} private daily summaries saved` : "Not connected"}</small></div><button className={connected ? "connected-button" : "outline-button"} onClick={()=>void (connected ? onCalendarDisconnect() : onCalendarConnect())}>{connected ? "Disconnect" : "Connect"}</button></div></section>}
...
function Dashboard({ exit, initialOnboarding = false, authenticated = false, calendarProviderToken = null }: { exit: () => void; initialOnboarding?: boolean; authenticated?: boolean; calendarProviderToken?: string | null }) {
  ...
  const connectCalendar = async () => { if (!authenticated) { setSyncMessage("Sign in first to connect your private calendar."); return; } localStorage.setItem("daymark-calendar-connect:v1", "pending"); const { error } = await getSupabaseBrowserClient().auth.signInWithOAuth({ provider: "google", options: { redirectTo: window.location.origin, scopes: "https://www.googleapis.com/auth/calendar.events.readonly", queryParams: { prompt: "consent", include_granted_scopes: "true" } } }); if (error) { localStorage.removeItem("daymark-calendar-connect:v1"); setSyncMessage("Google Calendar connection could not be started."); } };
